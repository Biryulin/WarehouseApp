...
namespace WarehouseApp.Forms
{
    public partial class MainFormManager : Form
    {
        ...
        public MainFormManager(int userId, string userName)
        {
            InitializeComponent();
            this.userId = userId;
            this.userName = userName;
            lblWelcome.Text = $"Добро пожаловать, {userName} (Менеджер)";
        }

        // НЕТ кнопки "Пользователи"
        private void btnInn_Click(object sender, EventArgs e)
        {
            TableForm form = new TableForm("ИНН_Справочник", userId, false);
            form.ShowDialog();
        }

        private void btnInventory_Click(object sender, EventArgs e)
        {
            TableForm form = new TableForm("Инвентаризация", userId, false);
            form.ShowDialog();
        }

        private void btnStockIncoming_Click(object sender, EventArgs e)
        {
            TableForm form = new TableForm("Приход_товара", userId, false);
            form.ShowDialog();
        }

        private void btnStockOutgoing_Click(object sender, EventArgs e)
        {
            TableForm form = new TableForm("Уход_товара", userId, false);
            form.ShowDialog();
        }
        ...
        private void InitializeComponent()
        {
            ...
            // btnInn
            // 
            this.btnInn.Location = new System.Drawing.Point(50, 80);
